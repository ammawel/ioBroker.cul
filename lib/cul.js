...
class Cul extends EventEmitter {
     ...
     */
    _init() {
        // Zuerst Version abfragen, um den Stick zu wecken
        this.write('V'); 
        
        // Etwas längere Pause, dann Empfang (X21) aktivieren
        setTimeout(() => {
            const cmd = this.options.initCmd || 'X21';
            // Wir loggen das intern für dich, falls du im Terminal schaust
            this.write(cmd);
            
            // Zweite Sicherheit: Wir senden X21 nach 1 Sekunde erneut
            setTimeout(() => {
                this.write(cmd);
                this.emit('ready');
            }, 1000);
